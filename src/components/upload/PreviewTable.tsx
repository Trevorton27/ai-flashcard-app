    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm ring-1 ring-zinc-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-xl font-semibold text-zinc-900">Review Vocabulary</h2>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-white bg-zinc-900 rounded-xl hover:bg-zinc-800 disabled:bg-zinc-300 disabled:cursor-not-allowed"
            >
        ...
        <div className="grid grid-cols-4 gap-4">
          <div className="bg-zinc-50 rounded-xl p-3 text-center">
            <div className="text-2xl font-semibold text-zinc-900">{result.stats.totalExtracted}</div>
            <div className="text-xs text-zinc-500">Extracted</div>
          </div>
          <div className="bg-green-50 rounded-xl p-3 text-center">
            <div className="text-2xl font-semibold text-green-700">{result.stats.translated}</div>
            <div className="text-xs text-green-600">Translated</div>
          </div>
          <div className="bg-yellow-50 rounded-xl p-3 text-center">
            <div className="text-2xl font-semibold text-yellow-700">{result.stats.duplicatesFound}</div>
            <div className="text-xs text-yellow-600">Duplicates</div>
          </div>
          <div className="bg-blue-50 rounded-xl p-3 text-center">
            <div className="text-2xl font-semibold text-blue-700">{unresolvedClarifications.length}</div>
      ...
      {unresolvedClarifications.length > 0 && (
        <div className="bg-yellow-50 rounded-2xl ring-1 ring-yellow-200 p-6">
          ...
          {tabs.map(tab => (
            ...
            >
              ...
              <span className={`ml-1.5 px-1.5 py-0.5 text-xs rounded-full ${
                activeTab === tab.id ? 'bg-zinc-900 text-white' : 'bg-zinc-100 text-zinc-600'
              }`}>
                {tab.count}
              </span>
            </button>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-zinc-50 border-b border-zinc-200">
                <th className="text-left text-xs font-semibold text-zinc-600 uppercase tracking-wider px-4 py-3">English</th>
                <th className="text-left text-xs font-semibold text-zinc-600 uppercase tracking-wider px-4 py-3">Japanese</th>
                <th className="text-left text-xs font-semibold text-zinc-600 uppercase tracking-wider px-4 py-3">Reading</th>
                <th className="text-left text-xs font-semibold text-zinc-600 uppercase tracking-wider px-4 py-3">Category</th>
                <th className="text-left text-xs font-semibold text-zinc-600 uppercase tracking-wider px-4 py-3">Status</th>
                <th className="text-right text-xs font-semibold text-zinc-600 uppercase tracking-wider px-4 py-3">Actions</th>
              </tr>
            </thead>
                ...
                filteredVocabulary.map((vocab, index) => {
                  ...
                  return (
                    <tr
                      ...
                      className={`${
                        vocab.needsClarification ? 'bg-yellow-50' :
                        isDuplicate ? 'bg-orange-50' :
                        duplicateAction === 'skip' ? 'bg-zinc-50 opacity-50' : ''
                      }`}
                    >
                      <td className="px-4 py-3 text-sm font-medium text-zinc-900">{vocab.english}</td>
                      <td className="px-4 py-3 text-sm text-zinc-900">{vocab.japaneseKanji || '—'}</td>
                      <td className="px-4 py-3 text-sm text-zinc-600">{vocab.hiragana || '—'}</td>
